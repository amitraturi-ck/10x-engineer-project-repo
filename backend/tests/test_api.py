...
def test_get_prompt_blank_id(client: TestClient):
    response = client.get("/prompts/ ")
    assert response.status_code == 404


def test_create_prompt_invalid_collection(client: TestClient, sample_prompt_data):
    bad_data = {**sample_prompt_data, "collection_id": "does-not-exist"}
    response = client.post("/prompts", json=bad_data)
    assert response.status_code == 400
    assert "Collection not found" in response.text


def test_update_prompt_not_found(client: TestClient):
    response = client.put(
        "/prompts/missing-id",
        json={"title": "X", "content": "Valid content here"}
    )
    assert response.status_code == 404


def test_update_prompt_invalid_collection(client: TestClient, sample_prompt_data):
    ...
    assert response.status_code == 400
def test_patch_prompt_success(client: TestClient, sample_prompt_data):
    create = client.post("/prompts", json=sample_prompt_data)
    prompt_id = create.json()["id"]
...
def test_patch_prompt_not_found(client: TestClient):
    response = client.patch(
        "/prompts/missing-id",
        json={"title": "X"}
    )
    assert response.status_code == 404
def test_list_prompts_filter_by_collection(client, sample_collection_data, sample_prompt_data):
    col = client.post("/collections", json=sample_collection_data)
...
def test_list_prompts_search(client, sample_prompt_data):
    client.post("/prompts", json=sample_prompt_data)

    response = client.get("/prompts?search=" + sample_prompt_data["title"][:4])
    assert response.status_code == 200
    assert response.json()["total"] == 1


def test_list_prompts_search_no_results(client):
    response = client.get("/prompts?search=nonexistenttext")
    assert response.status_code == 200
    assert response.json()["total"] == 0
def test_delete_prompt_not_found(client):
    response = client.delete("/prompts/missing-id")
    assert response.status_code == 404
def test_delete_collection_invalid_id(client):
    response = client.delete("/collections/ ")
    assert response.status_code == 400


def test_delete_collection_not_found(client):
    response = client.delete("/collections/missing-id")
    assert response.status_code == 404


def test_delete_collection_with_prompts_blocked(
    client, sample_collection_data, sample_prompt_data
):
    col = client.post("/collections", json=sample_collection_data)
    collection_id = col.json()["id"]
    ...
    assert response.status_code == 400
    assert "associated with existing prompts" in response.text
def test_create_prompt_empty_title(client):
...
def test_create_prompt_special_characters(client):
    response = client.post(
        "/prompts",
        json={"title": "🚀🔥", "content": "Valid content here"}
    )
